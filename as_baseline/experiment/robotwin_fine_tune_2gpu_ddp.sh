NCCL_SHM_DISABLE=1 .venv/bin/torchrun --nproc_per_node=2 --master_port=12342 -m scripts.train \
  --config=cosmos_predict2/_src/predict2/action/configs/action_conditioned/config.py \
  -- \
  experiment=robotwin_14d_finetune \
  job.name=robotwin_14d_finetune_pretrained_2gpu_ddp_smoke \
  trainer.max_iter=20 \
...
# CUDA_LAUNCH_BLOCKING=1 bash as_baseline/experiment/robotwin_fine_tune_2gpu_ddp.sh
