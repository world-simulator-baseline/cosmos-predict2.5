...
ACTION_EMBEDDER_INIT_METHOD="${ACTION_EMBEDDER_INIT_METHOD:-zero}"
case "$ACTION_EMBEDDER_INIT_METHOD" in
  random)
    ...
    DEFAULT_JOB_NAME=robotwin_14d_finetune_pretrained_4gpu_5task_random
    ;;
  zero)
    ACTION_EMBEDDER_TARGET=as_baseline.scripts.action_embedder_zero_init_weight_patch.RobotwinActionChunkConditionedDiT
    DEFAULT_JOB_NAME=robotwin_14d_finetune_pretrained_4gpu_5task_zero
    ;;
  *)
...
CHECKPOINT_SAVE_ITER="${CHECKPOINT_SAVE_ITER:-2000}"
...
.venv/bin/torchrun --nproc_per_node="$NUM_GPUS" --master_port="$MASTER_PORT" -m as_baseline.scripts.train_local_checkpoints_launcher \
  ...
  checkpoint.load_path=/data1/fangxuebin/models/cosmos_predict2_action_conditioned/robotwin/robotwin_14d_finetune_pretrained_4gpu_5task_zero/checkpoints/iter_000003062 \
  ...
  2>&1 | tee -a train_zero_5task.log
